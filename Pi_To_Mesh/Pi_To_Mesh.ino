...
void loop() {
  while(Serial.available()) {
    ...
    uint8_t type = strtoul(inputChar, &pEnd, 16);
    uint32_t dest = strtoul(pEnd, &pEnd, 16);
    uint8_t cmd = strtoul(pEnd, NULL, 16);
    
    fire(type, dest, cmd);
    
    Serial.println("String received -> " + input + " Length: " + input.length());

    Serial.println("Type: " + String(type) + " (0x" + String(type, HEX) + ")");
    Serial.println("Dest: " + String(dest) + " (0x" + String(dest, HEX) + ")");
    Serial.println("Cmd:  " + String(cmd)  + " (0x" + String(cmd,  HEX) + ")");
    Serial.println("");
